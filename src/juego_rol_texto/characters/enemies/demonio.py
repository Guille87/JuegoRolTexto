    ...
    def drop_item(self) -> list:
        ...
        if random.random() <= 0.08:
            items.append(Armor("Coraza Demoníaca", "Placas de hueso ennegrecido, calientes al tacto.", 80,
                                slot="peto", defense=23, magic_resist=6, max_health=30))
        if random.random() <= 0.08:
            items.append(Weapon("Garra Infernal", "Arrancada de un demonio menor; sigue chisporroteando.", 45, 29, element="fuego"))
        if random.random() <= 0.08:
            items.append(Armor("Perneras Infernales", "Cubiertas de brasas que nunca terminan de apagarse.", 60,
                                slot="perneras", evasion=6, defense=10, magic_resist=4))
        return items
