...
**Known pre-existing quirk (preserved intentionally, not a regression)**: `Weapon.use()`, `Armor.use()`, and `HealingPotion.use()` don't `return True`. `Inventory._handle_selection()` treats a falsy return as "the action failed" and returns `False` — so equipping a weapon/armor or drinking a healing potion **does** apply its effect but is reported back as "not used" (e.g. the item is never decremented from the stack, and in battle it never consumes a turn as `"objeto_usado"`). Only `StatBuffPotion`/`RegenPotion` return `True`/`False` correctly. This was true before the refactor and was deliberately left as-is (see tests in `tests/test_inventory.py` documenting the exact behavior). Don't "fix" this without checking with the user first — it may be relied upon (e.g. healing potions being effectively free/non-consumed could be intentional balance).

**Inventory (`inventory/inventory.py`)**: consumables stack via a `quantities` dict keyed by item name; `Weapon`/`Armor` don't stack — picking up a duplicate auto-sells it for gold instead. `show_inventory(filter_class, mode)` is reused both for read-only viewing and interactive selection (`mode="use"`), filtered by class for the weapon/armor equip menus.
